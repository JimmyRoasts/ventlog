...
import type { LayoutData } from './$types';
	...
	const navItems = [
		...
		{ name: 'Survey points', href: '/app/nodes' },
...
<div class="min-h-screen flex flex-col bg-background text-foreground transition-colors">
	<header class="border-b border-border/70 bg-card/80 backdrop-blur">
		<div class="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
			<a href="/app/dashboard" class="flex items-center gap-2 text-lg font-semibold tracking-tight text-foreground">
				<span aria-hidden="true" class="text-primary">◆</span>
				<span>Ventlog</span>
			</a>
			<nav class="hidden items-center gap-2 rounded-full bg-muted/60 px-2 py-1 text-sm font-medium text-muted-foreground sm:flex">
	...
	<main class="flex-1 mx-auto max-w-6xl w-full px-4 py-10 sm:px-6">
		<div class="mb-4 flex gap-3 text-sm font-medium text-muted-foreground sm:hidden">
			{#each navItems as item}
				{@const active = $page.url.pathname.startsWith(item.href)}
				<a class={`rounded-md px-2 py-1 ${active ? 'bg-primary/15 text-foreground' : 'hover:text-foreground'}`} href={item.href}>
					{item.name}
				</a>
			{/each}
		</div>
	...
	<footer class="border-t border-border/60 bg-card/80 text-xs text-muted-foreground">
		<div class="mx-auto flex max-w-6xl w-full flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
